    ...
    internal static class ApplicationExtensionMethods
    {
        ...
        internal static void SetTableRelationProperty(this TableRelationLinesProperty property, string propertyValue)
        {
            while (propertyValue.Length > 0)
            {
                ...
                {
                    tableRelationLine.Conditions.Add(condition.FieldName, condition.Type.ToEnum<TableRelationConditionType>(), condition.Value);
                }

                foreach (var filter in filters)
                {
                    tableRelationLine.TableFilter.Add(filter.FieldName, filter.Type.ToEnum<TableRelationTableFilterLineType>(), filter.Value);
                }
        ...
        internal static void SetCalcFormulaProperty(this CalcFormulaProperty property, string propertyValue)
        {
            ...
            foreach (var filter in filters)
            {
                var calcFormulaTableFilterLine = property.Value.TableFilter.Add(new CalcFormulaTableFilterLine(filter.FieldName, filter.Type.ToEnum<CalcFormulaTableFilterType>(), filter.Value));
        ...
        internal static void SetDataItemQueryElementTableFilter(this DataItemQueryElementTableFilterProperty property, string propertyValue)
        {
            ...
            {
                var fieldName = Parsing.MustMatch(ref propertyValue, @"^([^=]+)=").Groups[1].Value;
                var type = Parsing.MustMatch(ref propertyValue, @"^(CONST|FILTER)").Groups[1].Value.ToEnum<DataItemQueryElementTableFilterLineType>();
